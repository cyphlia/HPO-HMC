...
* **What to Cover:**
  - **Method A: Pure HHD (Continuous HMC)**
  ...
  - **Method C: Unified HHD-ABBO (Our Contribution)**
    - A novel **three-phase epoch curriculum** that fuses both philosophies.

---

## Slide 5: Method C — The Three-Phase Curriculum
...
* **What to Cover:**
  ...
  - **Results Table:**
    - Method C achieves a validation loss of **`0.003270`** (**30x better** than Method B's `0.098957`).
    - Method C achieves a landscape MAE of **`0.017673`** (**6x better** than Method B's `0.104961`).
    - Method C achieves a near-perfect R² score of **`0.999950`**.
...
## Slide 7: Benchmark Results — MNIST CNN
* **What to Cover:**
  - **Setup:** Subsampled MNIST training set (5,000 samples) to enforce overfitting pressure.
  - **Results Table:**
    - **HHD (Method A):** 97.80% validation accuracy, completed in **122.7 seconds**.
    - **ABBO (Method B):** 97.40% validation accuracy, completed in **698.7 seconds**.
    - **Unified (Method C):** 97.70% validation accuracy, completed in **214.8 seconds**.
  - **Takeaway:** Method C achieves comparable accuracy to HHD/ABBO but runs **over 3.2x faster than Method B**, eliminating the need for expensive multi-trial network re-initializations.
...
* **What to Cover:**
  ...
  - **Average Rankings (Lower is better):**
    1. **Method C (Unified):** **2.45** (Best)
    2. **Optuna TPE:** 2.64
    3. **Method A (HHD):** 3.18
    4. **Method B (ABBO):** 3.18
    5. **Random Search:** 3.55
  - **Takeaway:** Method C generalizes robustly across different datasets and tabular search spaces, outperforming industry-standard Optuna TPE.
...
### Q2: Why is Method C's actual validation loss (0.003270) so much better than what was logged in its epoch history?
...
### Q3: Why does Method B have a worse MAE (0.1049) than Method C (0.0176) even though its training loss converges?
* **Answer:** The training and validation datasets contain target noise ($\sigma = 0.05$). Method B's GP surrogate directly minimizes this noisy loss, causing it to overfit the noise. Method C uses physical constraints and regularization during HMC exploration, allowing it to generalize past the noise and reconstruct the *noiseless* mathematical landscape.
